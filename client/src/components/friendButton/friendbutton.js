...
export default function FriendButton({ otherUserId }) {
    ...
    useEffect(() => {
        ...
        axios.get(`/friendshipStatus/${otherUserId}`).then(({ data }) => {
            ...
            console.log(
                "data get friendshipStatus: ",
                data.friendshipStatus[0]
            );
            ...
            if (!data.friendshipStatus[0]) {
                ...
                data.friendshipStatus[0].sender_id == data.userId
                    ? setButtonText(PATTERN.CANCEL)
                    : setButtonText(PATTERN.ACCEPT);
            }
        });
    }, [button]);
    ...
    function handleClick() {
        ...
        if (button == PATTERN.SEND) {
            axios
                .post(`/sendFriendRequest/${otherUserId}`)
                .then((res) => {
        ...
        if (button == PATTERN.END) {
            axios
                .post(`/deleteFriendship/${otherUserId}`)
                .then((res) => {
        ...
        if (button == PATTERN.CANCEL) {
            axios
                .post(`/cancelFriendRequest/${otherUserId}`)
                .then((res) => {
